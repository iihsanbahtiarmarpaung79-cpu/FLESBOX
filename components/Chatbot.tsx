...
export const Chatbot: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', text: 'Hai! Saya FLESBOX AI Tutor. Mau tanya soal Matematika, IPA, atau Fiqih? Tulis aja ya, saya siap bantu!' }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'model', text: response || 'Maaf, saya sedang memproses banyak data. Bisa ulangi?' }]);
    } catch (err) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[60] flex flex-col items-end">
      {isOpen && (
        <div className="bg-white w-[calc(100vw-3rem)] sm:w-96 h-[70vh] sm:h-[500px] rounded-[32px] shadow-2xl flex flex-col border border-indigo-50 overflow-hidden animate-in slide-in-from-bottom-10 mb-4 glass-morphism">
          ...
          <div className="p-5 bg-white border-t border-slate-100 flex items-center space-x-3">
            <input
              type="text"
              value={input}
